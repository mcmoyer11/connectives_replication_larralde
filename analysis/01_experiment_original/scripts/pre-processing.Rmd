...
length(unique(d$MD5.hash.of.participant.s.IP.address)) #80
...
dd <- d %>%
  # training, filler and test items
  filter(Label %in% c("test","warmup1","warmup2")) %>% 
  select(MD5.hash.of.participant.s.IP.address,Connective,ReactionTime1,ReactionTime2,ReactionTime3,TrialID,Label,Wordform,Answeraccuracy,Truthvalue,Sentenceform,Group) %>% 
  group_by(MD5.hash.of.participant.s.IP.address,TrialID) 

# remove the duplicate rows
d.pretest <- dd %>% 
  unique()

# Sanity check that no participants were removed yet
length(unique(dd$MD5.hash.of.participant.s.IP.address))

length(unique(d.pretest$MD5.hash.of.participant.s.IP.address)) # 80 subjects as expected

table(d.pretest$Label)
table(d.pretest$Label,d.pretest$MD5.hash.of.participant.s.IP.address)
```

# Take a look at the training items
```{r}
d.training <- d %>% 
  select(MD5.hash.of.participant.s.IP.address,Label,Order.number.of.item,PennElementName,Value) %>%
  filter(Label %in% c("warmup1","warmup2")) %>% 
  filter(Value %in% c("F","J"))

length(unique(d.training$MD5.hash.of.participant.s.IP.address)) # 80

# How many training items per participant
table(d.training$MD5.hash.of.participant.s.IP.address,d.training$Label)

d.sum <- d.training %>% 
  filter(Label == "warmup2") %>% 
  group_by(MD5.hash.of.participant.s.IP.address) %>%
  mutate(count = n(),prop = count/30) %>% 
  select(-c(Order.number.of.item,PennElementName,Value,Label)) %>% 
  unique()
# View(d.sum)  
...
ggplot(data=d.sum, aes(x=reorder(MD5.hash.of.participant.s.IP.address,count),y=count)) +
  geom_bar(position=dodge,stat="identity") +
  ggtitle("Original Study: number of training trials per participant")

ggsave("../graphs/original_training_items.pdf")

```

## Retrieve the participant ID info
```{r}

d.IDs <- d %>% 
  select(ID, MD5.hash.of.participant.s.IP.address) %>% 
  filter(!ID %in% c("NULL","Wait success","Shift",
                    "CapsLock",
                    "b","a","f","s","-","t","n","y","z","e","l",
                    "l","i","w","'","/")) %>% 
  unique()
