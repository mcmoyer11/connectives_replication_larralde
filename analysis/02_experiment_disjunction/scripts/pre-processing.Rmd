...
ggplot(data=d.sum, aes(x=reorder(MD5.hash.of.participant.s.IP.address,count),y=count)) +
  ...
  ggtitle("Disjunction Study: number of training trials per participant")

ggsave("../graphs/disjunction_training_items.pdf")
...
length(unique(d.pretest$ID)) # 80 subjects as expected
...
# Take a look at the training items
```{r}
d.training <- d.pretest %>% 
  filter(Label %in% c("warmup1","warmup2"))
length(unique(d.training$ID))
